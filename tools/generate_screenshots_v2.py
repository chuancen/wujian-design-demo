...
OUT = ROOT / "screenshots-v2"
SVG_OUT = ROOT / "screenshot-sources-svg-v2"
W, H = 1440, 1440
...
def frame(s, theme_key, page):
    ...
    t = s.t
    if theme_key == "A":
        s.rect(0, 0, 230, H, fill=t["dark"], rx=0)
        s.text(28, 52, "五健体检平台", 25, "#fff", 900)
        nav = ["工作台", "体检任务", "采集管理", "五健专项", "儿童档案", "基础信息", "随访复查", "统计分析", "系统集成", "系统设置"]
        active = nav[(int(code[1:]) - 1) % len(nav)]
        for i, item in enumerate(nav):
            yy = 104 + i * 54
            if item == active:
                s.rect(22, yy - 30, 186, 42, fill="#2d74c8", rx=6)
            s.text(52, yy, item, 17, "#fff" if item == active else "#bed5ef", 800 if item == active else 550)
        s.rect(230, 0, W - 230, 76, fill="#fff", rx=0)
        s.text(260, 48, f"{code} {title}", 25, t["text"], 900)
        s.badge(1112, 24, "医院管理员", fill="#eaf3ff", color=t["primary"], w=116)
        s.badge(1242, 24, "杭州院区", fill="#eef8f6", color=t["accent"], w=104)
        s.para(260, 112, f"验收点：{proof}", 65, 17)
        return (260, 150, 1128, 810)
    if theme_key == "B":
        s.rect(28, 24, 1384, 78, fill=t["dark"], stroke=t["line"], rx=4)
        s.text(64, 72, t["brand"], 26, t["primary"], 900)
        s.text(720, 72, f"{code} {title}", 30, t["text"], 900, anchor="middle")
        s.badge(1230, 48, "安全只读", fill=t["soft"], color=t["accent"], w=118)
        s.para(54, 138, f"态势说明：{proof}", 80, 17)
        return (54, 178, 1332, 820)
    if theme_key == "C":
